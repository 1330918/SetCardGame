...
class Game():
    ...
    def change_cards(self):
        ...
        for i in range(3):
            card = pg.image.load("Cards/" + cards[0] + ".png")
        ...
        while len(self.sets) == 0 and len(cards) > 0:
            ...
            for k in range(3, 12, 4):
                extra_card = pg.image.load("Cards/" + cards[0] + ".png")
...
for n in range(12):
    card_number_label = Label(f"{n+1}", (margins[n][0]-15, margins[n][1]+15), black, (238,224,229), "berlinsansfb", 18, (25,25), italic=True)
    card_number_label.draw()
